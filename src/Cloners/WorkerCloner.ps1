function Copy-OctopusWorkers
{
    ...
    {                              
        ...
        {            
            ...
            $copyOfItemToClone.MachinePolicyId = Convert-SourceIdToDestinationId -SourceList $sourceData.MachinePolicyList -DestinationList $destinationData.MachinePolicyList -IdValue $worker.MachinePolicyId -ItemName "$($copyOfItemToClone.Name) Machine Policy" -ThrowErrorOnMismatch $true
...
function Get-OctopusFilteredWorkerPoolIdList
{
    ...
    foreach ($workerPoolId in $worker.WorkerPoolIds)
    {
        ...
        $destinationWorkerPoolId = Convert-SourceIdToDestinationId -SourceList $sourceData.WorkerPoolList -DestinationList $destinationData.WorkerPoolList -IdValue $workerPoolId -ItemName "$($worker.Name) Worker Pool" -ThrowErrorOnMismatch $true
